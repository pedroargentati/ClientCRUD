...
import br.com.fiap.jdbcadapter.DataAccessObjectAdapter;
		...
		try {
			...
			try {
				...
				while (resultSet.next()) {
					...
					vo.setNome_cliente	   ( resultSet.getString("NM_CLIENTE") );
					vo.setEmail			   ( resultSet.getString("EMAIL") );
					vo.setLogradouro	   ( resultSet.getString("LOGRADOURO") );
					vo.setComplemento	   ( resultSet.getString("COMPLEMENTO") );
					vo.setCep			   ( resultSet.getString("CEP") );
					vo.setId_cidade   	   ( resultSet.getInt	("ID_CIDADE") );
	...
	public void alterarCliente(ClienteVo clienteVo) throws SQLException {
		try {
			...
			try {
				...
				preparedStatement.setString	(	2, clienteVo.getEmail() );
				preparedStatement.setString	(	3, clienteVo.getLogradouro());
				preparedStatement.setString	(	4, clienteVo.getComplemento() );
				preparedStatement.setString	(	5, clienteVo.getCep() );
				preparedStatement.setInt	(	6, clienteVo.getId_cidade() );
				preparedStatement.setString	(	7, clienteVo.getUf() );
				preparedStatement.setString	(	8, clienteVo.getTp_cliente() );
				preparedStatement.setInt	(	9, clienteVo.getId_cliente() );
				preparedStatement.execute();
